 ...
 * 脚本。完整复现步骤 + 修复建议见 docs/03-database/BUG_REPORT_SYNC_EVENT_APPLY_FUNCTIONS_2026-07-19.md。
 * 下面的 `test.fails(...)` 用例刻意断言"正确"行为（不应重复扣减），当前会按预期失败——
 * 这是有意为之的活文档：一旦 DBA 修正该函数加上行锁，此用例会转为意外通过（unexpected
 * pass），vitest 会让整个套件报错，提醒把 `test.fails` 改回普通 `test`，从而在不引入
 * 额外看板的情况下追踪该已知问题是否已修复。
 *
 * 已知的时序抖动（2026-07-19 实测记录）：本地反复运行观察到约 1/7 的概率这条 `test.fails`
 * 会"意外通过"（即两次并发调用这次没有真正抢到同一个时间窗口，没触发退化）——这是并发
 * bug 测试本身的固有属性（依赖真实网络/调度时序，不是每次都能命中最窄的竞态窗口），
 * 不代表 bug 已修复。如果看到这条用例报"Expect test to fail"，先连续重跑几次确认是否
 * 稳定转绿，而不是直接认定已经修好。真正修复后应该是稳定 100% 意外通过。
 *
 * 下面还有一条同类型的 `test.fails(...)`，对应同一批文件里另一个独立问题（未知
 * action_type 场景没有登记进统一异常领域），详见该用例前的注释和上述 bug 报告文档。
 *
...
describe.skipIf(!RUN)('SupabaseSyncEventRepository 同步事件收件箱正确性（Phase 5/6/7 P0 第 2 项）', () => {
  ...
  // 有意标记为"预期失败"的活文档，见文件头部说明
  test.fails(
    '[已知问题-待DBA修正] applyEvent 对未知 action_type 应在统一异常域登记 SYNC_APPLY_FAILURE（当前只改状态，不登记异常）',
    async () => {
      ...
      // Assert（当前会失败：fn_apply_sync_event 的 ELSE 分支只把 sync_events.status 改成
      // REJECTED，没有像同一函数里 WMS01/OTHERS 两个分支那样调用 fn_raise_exception，
      // 导致这类"设备发了个系统不认识的动作类型"的场景完全不在 exceptions 表里出现，
      // 运维在 GET /exceptions 上永远看不到，违反了"统一异常领域"覆盖所有需关注场景的设计目标）
