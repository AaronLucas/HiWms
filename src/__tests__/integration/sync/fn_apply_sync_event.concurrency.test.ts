...
describe.skipIf(!RUN)('SupabaseSyncEventRepository 同步事件收件箱正确性（Phase 5/6/7 P0 第 2 项）', () => {
  ...
  test('applyEvent：库存不足时不应静默标记为 APPLIED，应保留数据库函数已登记的 EXCEPTION 状态', async () => {
    ...
    // Assert（CRITICAL 缺口回归防护，见 REPOSITORY_ROADMAP.md §「剩余缺口清单」）：
    // applyEvent 的返回值必须带上 fn_raise_exception 实际创建的那条 exceptions.id，
    // 而不是把 exceptionId 永远留空——processPendingEvents 的批量汇总依赖这个字段
    // 才能把"哪个事件对应哪条异常"关联起来返回给调用方。
    const { data: exceptionRow, error: excErr } = await client
      .from('exceptions')
      .select('id')
    ...
    expect(result.exceptionId).toBe(exceptionRow!.id);
  });
  ...
  test('processPendingEvents（CRITICAL 缺口回归防护，见 REPOSITORY_ROADMAP.md §「剩余缺口清单」）：批量处理应正确汇总 processed/succeeded/failed，且 exceptions 数组应带上真实 exceptionId 而非永远为空', async () => {
    // Arrange：全新租户级隔离范围，避免与其他用例遗留的 PENDING 事件混入同一次批量拉取
    const { data: pTenant, error: tenantErr } = await client
      .from('tenants')
      .insert({ name: `ecc-p0-2-batch-tenant-${Date.now()}` })
      .select()
      .single();
    if (tenantErr) throw tenantErr;

    try {
      const { data: pDevice, error: deviceErr } = await client
        .from('devices')
        .insert({ tenant_id: pTenant.id, device_code: `ecc-p0-2-batch-device-${Date.now()}`, device_type: 'PDA' })
        .select()
        .single();
      if (deviceErr) throw deviceErr;

      const { data: pProduct, error: productErr } = await client
        .from('products')
        .insert({ tenant_id: pTenant.id, sku: 'P0-2-BATCH-SKU', name: 'P0 Batch Test Product' })
        .select()
        .single();
      if (productErr) throw productErr;

      const { data: pLocation, error: locationErr } = await client
        .from('locations')
        .insert({ tenant_id: pTenant.id, code: `p0-2-batch-loc-${Date.now()}`, is_active: true })
        .select()
        .single();
      if (locationErr) throw locationErr;

      const { data: pContainer, error: containerErr } = await client
        .from('containers')
        .insert({ lpn_code: `p0-2-batch-lpn-${Date.now()}`, current_location_id: pLocation.id })
        .select()
        .single();
      if (containerErr) throw containerErr;

      const { error: inventoryErr } = await client
        .from('inventory')
        .insert({ tenant_id: pTenant.id, product_id: pProduct.id, location_id: pLocation.id, container_id: pContainer.id, quantity: 5 });
      if (inventoryErr) throw inventoryErr;

      const makeEvent = (seq: number, qty: number) => ({
        id: randomUUID(),
        tenant_id: pTenant.id,
        device_id: pDevice.id,
        device_seq: seq,
        action_type: 'PICK',
        payload: { sku: 'P0-2-BATCH-SKU', qty, location_id: pLocation.id, container_id: pContainer.id },
        captured_at: new Date().toISOString(),
        status: 'PENDING',
      } as SyncEventInsert);

      // 一个库存充足（应 APPLIED），一个库存不足（应 EXCEPTION 并登记异常）
      const succeedEvent = makeEvent(1, 2);
      const failEvent = makeEvent(2, 100);
      const { error: insertErr } = await client.from('sync_events').insert([succeedEvent, failEvent]);
      if (insertErr) throw insertErr;

      // Act
      const summary = await repo.processPendingEvents(pTenant.id);
      ...
      // Assert：失败事件应出现在 exceptions 数组里，且 exceptionId 必须是 exceptions 表里
      // 真实存在的那一条记录的 id——修复前 applyEvent 从不回传 exceptionId，这里永远是
      // 空数组，调用方拿不到任何异常关联信息。
      expect(summary.exceptions).toHaveLength(1);
      expect(summary.exceptions[0].eventId).toBe(failEvent.id);
      expect(summary.exceptions[0].exceptionId).toBeTruthy();

      const { data: exceptionRow, error: excErr } = await client
        .from('exceptions')
        .select('id')
        .eq('source_table', 'sync_events')
        .eq('source_id', failEvent.id)
        .single();
      expect(excErr).toBeNull();
      expect(summary.exceptions[0].exceptionId).toBe(exceptionRow!.id);
    } finally {
      await client.from('tenants').delete().eq('id', pTenant.id);
    }
  });

  test('markStalledEventAsException（HIGH 缺口回归防护，见 REPOSITORY_ROADMAP.md §「剩余缺口清单」）：仍处于 PENDING 的事件应被标记为 EXCEPTION', async () => {
    // Arrange：直接插入一条 PENDING 事件，不经过 applyEvent（模拟"RPC 调用本身抛出异常，
    // fn_apply_sync_event 从未真正跑起来"的场景——这种场景难以通过真实网络故障稳定复现，
    // 改为直接对 applyEvent 抽出的收尾方法本身做测试）
    const eventId = randomUUID();
    const { error } = await client.from('sync_events').insert({
      id: eventId,
      tenant_id: tenantId,
    ...
    // Act：私有方法，测试内显式访问以验证这段此前零覆盖的收尾逻辑
    const markError = await (repo as unknown as { markStalledEventAsException(id: string): Promise<string | undefined> }).markStalledEventAsException(eventId);

    // Assert
    expect(markError).toBeUndefined();
    const { data: row, error: readErr } = await client.from('sync_events').select('status, applied_at').eq('id', eventId).single();
    expect(readErr).toBeNull();
    expect(row!.status).toBe('EXCEPTION');
    expect(row!.applied_at).not.toBeNull();
  });

  test('markStalledEventAsException：非 PENDING 状态（已由 SQL 函数自行落定）不应被覆盖', async () => {
    // Arrange：一个已经真实 APPLIED 的事件（服务端其实已经提交，只是假设客户端这边
    // 因超时/连接中断误以为失败）
    const locationId = await createLocation();
    const containerId = await createContainer(locationId);
    await seedInventory(locationId, containerId, 50);
    const eventId = await createPickEvent(locationId, containerId, 5);
    const applied = await repo.applyEvent(eventId);
    expect(applied.success).toBe(true);
    const before = await client.from('sync_events').select('status, applied_at').eq('id', eventId).single();
    expect(before.data!.status).toBe('APPLIED');

    // Act
    const markError = await (repo as unknown as { markStalledEventAsException(id: string): Promise<string | undefined> }).markStalledEventAsException(eventId);

    // Assert：不应被悄悄覆盖成 EXCEPTION，也不应改动 applied_at
    expect(markError).toBeUndefined();
    const after = await client.from('sync_events').select('status, applied_at').eq('id', eventId).single();
    expect(after.data!.status).toBe('APPLIED');
    expect(after.data!.applied_at).toBe(before.data!.applied_at);
  });
});
