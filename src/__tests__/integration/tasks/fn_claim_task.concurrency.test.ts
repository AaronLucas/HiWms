...
describe.skipIf(!RUN)('SupabaseTaskClaimRepository 竞争性任务领用正确性（Phase 5/6/7 P0）', () => {
  ...
  test('extendLease：正常续租应把 expires_at 精确延长 additionalSeconds', async () => {
    // Arrange
    const workOrderId = await createWorkOrder();
    const claimed = await repo.claimTask({
      workOrderId,
    ...
    expect(claimed?.success).toBe(true);
    const before = await client.from('task_claims').select('expires_at').eq('id', claimed!.claimId).single();
    expect(before.error).toBeNull();

    // Act
    const updated = await repo.extendLease(claimed!.claimId, 120);

    // Assert
    expect(updated).not.toBeNull();
    const expectedMs = new Date(before.data!.expires_at).getTime() + 120 * 1000;
    expect(new Date(updated!.expires_at).getTime()).toBe(expectedMs);
  });

  test('extendLease（HIGH 缺口回归防护，见 REPOSITORY_ROADMAP.md §「剩余缺口清单」）：对同一租约真实并发续租 5 次，应全部生效而不是互相覆盖丢失更新', async () => {
    // Arrange
    const workOrderId = await createWorkOrder();
    const claimed = await repo.claimTask({
      workOrderId,
    ...
    // Act：5 个真正并发（Promise.all）的续租请求，每次 +60 秒
    const results = await Promise.all([0, 1, 2, 3, 4].map(() => repo.extendLease(claimed!.claimId, 60)));

    // Assert：如果是读改写竞态（未修复前的实现），5 次并发很可能只有部分甚至只有 1 次
    // 真正生效；乐观重试修复后，5 次都应成功返回非 null，且最终 expires_at 精确等于
    // 原始值 + 5*60 秒，不多不少——不是"任意一次中间结果"。
    expect(results.every((r) => r !== null)).toBe(true);
    const after = await client.from('task_claims').select('expires_at').eq('id', claimed!.claimId).single();
    expect(after.error).toBeNull();
    const expectedMs = new Date(before.data!.expires_at).getTime() + 5 * 60 * 1000;
    expect(new Date(after.data!.expires_at).getTime()).toBe(expectedMs);
  });

  test('extendLease：对已失效（非 ACTIVE）的租约应返回 null，不应把已到期的租约悄悄复活', async () => {
    // Arrange：制造一个已过期并已被清扫为 EXPIRED 的租约
    const workOrderId = await createWorkOrder();
    const claimed = await repo.claimTask({
      workOrderId,
      userId: userIds[0],
    ...
    expect(claimed?.success).toBe(true);
    await repo.expireTaskClaims();
    const before = await client.from('task_claims').select('expires_at, status').eq('id', claimed!.claimId).single();
    expect(before.data!.status).toBe('EXPIRED');

    // Act
    const updated = await repo.extendLease(claimed!.claimId, 300);

    // Assert：不应返回更新后的行，也不应修改 expires_at
