...
describe.skipIf(!RUN)('SupabaseTenantRepository 租户 CRUD 正确性（Phase 8）', () => {
  ...
  test('findByName：按名称查找租户', async () => {
    ...
    const notFound = await repo.findByName('NONEXISTENT_TENANT_NAME');
    expect(notFound).toBeNull();
  });

  test('findActive：只返回启用中的租户', async () => {
    const inactiveName = `TEST_INACTIVE_${randomUUID().slice(0, 8)}`;
    const inactive = await repo.create({ name: inactiveName, is_active: false });
    createdTenantIds.push(inactive.id);

    const activeName = `TEST_ACTIVE_${randomUUID().slice(0, 8)}`;
    const active = await repo.create({ name: activeName, is_active: true });
    createdTenantIds.push(active.id);

    const activeTenants = await repo.findActive();
  ...
  test('update：更新租户字段', async () => {
    ...
    const updated = await repo.update(created.id, { name: newName, is_active: false });
  ...
  test('updateBillingStrategy：更新计费策略', async () => {
    ...
    const updated = await repo.updateBillingStrategy(created.id, strategy);
  ...
  test('并发创建：不同名称并发创建不冲突', async () => {
    ...
    const results = await Promise.all(names.map((name) => repo.create({ name })));
