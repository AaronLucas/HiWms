...
describe.skipIf(!RUN)('SupabaseExceptionRepository 统一异常领域正确性（Phase 5 第 5 项）', () => {
  ...
  beforeAll(async () => {
    ...
    const { data: authUser, error: authUserErr } = await client
      .from('users')
      .insert({ tenant_id: tenantId, username: `exc-auth-user-${Date.now()}`, password_hash: '$2b$12$LJ3m4ys3LkBCVxJGqOjPkuF0mJqVxJGqOjPkuF0mJqVxJGqOjPku' })
      .select()
      .single();
    ...
    const { data: unauthUser, error: unauthUserErr } = await client
      .from('users')
      .insert({ tenant_id: tenantId, username: `exc-unauth-user-${Date.now()}`, password_hash: '$2b$12$LJ3m4ys3LkBCVxJGqOjPkuF0mJqVxJGqOjPkuF0mJqVxJGqOjPku' })
      .select()
      .single();
