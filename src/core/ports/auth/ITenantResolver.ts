...
export interface ITenantResolver {
   ...
   */
  resolveFromUser(userId: string, authToken?: string): Promise<string | null>;
   ...
   * 验证租户是否存在且激活
   */
  validateTenant(tenantId: string, authToken?: string): Promise<boolean>;
   ...
   */
  isPlatformAdmin(userId: string, authToken?: string): Promise<boolean>;
}