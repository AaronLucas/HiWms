...
> **同步来源**: `supabase/migrations/` 下的初始化脚本（V2.1 主脚本 + 离线同步/统一异常领域增量扩展 + 同步动作扩展 + 追踪策略/无码货物）。**注意**：`supabase/` 目录已按项目决定加入 `.gitignore`（不再纳入版本管理），因此本文档而非某个具体 SQL 文件才是表结构的版本化事实来源；SQL 迁移脚本的实际落地由部署流程另行维护。  
> **状态**: ✅ V2.1 主体生产就绪；🚧 Layer 2（离线同步/统一异常领域）设计已确定、待迁移脚本落地；🚧 Layer 3（同步动作扩展）+ Layer 4（追踪策略/无码货物）设计已确定、待迁移脚本落地（工程排期见 `docs/00-project/ROADMAP.md` Phase 1.4、`docs/03-database/REPOSITORY_ROADMAP.md` Phase 5-7）

---

## 0. 本次变更说明（v2.3.0）

DBA 团队评审了旧版 PDA 离线同步设计后，交付了四层增量扩展方案（详见 `docs/01-architecture/PDA_OFFLINE_SYNC_DESIGN.md`）。本次更新把 **Layer 3（同步动作扩展）** 与 **Layer 4（追踪策略 + 无码/未识别货物）** 的新表、新函数、RLS、索引、约束全部补充进本文档：

- **Layer 3 新表**：`packing_task_items`、`inventory_count_policies`（容差配置）
- **Layer 3 新函数**：`fn_adjust_inventory_at_location`、`fn_reconcile_location_count`、`fn_apply_putaway_action`、`fn_apply_count_action`、`fn_apply_pack_action`、修正合规触发器
- **Layer 4 新表**：`tenant_tracking_policies`
- **Layer 4 新函数**：`fn_generate_internal_lpn`、`fn_confirm_label_applied`、`fn_identify_unidentified_goods`、`fn_receive_unidentified_goods`、`fn_requires_unique_tracking`、合规触发器扩展到 `UPDATE OF product_id`
- 新异常类型：`MISSING_LABEL`、`UNIDENTIFIED_GOODS`、`REFERENCE_NOT_FOUND`（加入 `exception_type_catalog`）
- `inventory_reservations` 增加 `work_order_id` 列（库存预占精确到工单）
- `order_lines.status` CHECK 约束增加 `EXCEPTION` 取值（Layer 2 已有，本次重申）

设计原则与业务背景见 `PDA_OFFLINE_SYNC_DESIGN.md`、`SYNC_ACTIONS_EXTENSION.md`、`TRACKING_POLICY_MISSING_LABEL.md`；本文档只记录表结构事实，不重复设计动机。

---

## 1. 表结构总览（47 张业务表：38 张 V2.1 核心表 + 7 张离线同步/异常领域表 + 2 张 Layer 3/4 新表）
...
| | `packing_task_items` | 打包明细（订单行、数量、箱子、容器，**Layer 3 新增**，支持一箱一码/同码模式） | ✅ | ✅ | - |
...
| **统一异常领域** | `exception_type_catalog` | 异常类型元数据字典（domain、默认严重度、处理所需权限，支持全局默认+租户覆盖，**Layer 3/4 新增 MISSING_LABEL/UNIDENTIFIED_GOODS/REFERENCE_NOT_FOUND**） | ✅ (特殊策略，见 §8) | ✅ | ✅ |
...
| **Layer 3 同步动作扩展** | `packing_task_items` | 打包明细（订单行、数量、箱子、容器，支持一箱一码/同码模式，**Layer 3 新增**） | ✅ | ✅ | ✅ |
| | `inventory_count_policies` | 盘点容差策略（租户默认值 + SKU 级覆盖，**Layer 3 新增**） | ✅ | ✅ | ✅ |
| **Layer 4 追踪策略** | `tenant_tracking_policies` | 租户 ABC 类追踪默认策略（A=追踪/C=不追踪/B=显式配置，**Layer 4 新增**） | ✅ | ✅ | ✅ |
