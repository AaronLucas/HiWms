...
## 推荐方案（2026-07-28 补充，供项目负责人/DBA 参考，不代为最终决定）

以下是开发团队对 5 项未决问题中 4 项给出的推荐方案 + 理由，目的是降低拍板时的
讨论成本；**#3 涉及现有生产/测试数据现状，只有项目负责人能确认，这里不给推荐**。

1. **`email` 唯一约束范围** → 推荐**全局唯一**（不做跨租户维度区分）。
   Supabase Auth 的 `auth.users.email` 本身默认就是全局唯一的（一个邮箱只能注册
   一个 Auth 账号），`public.users.email` 跟随这个语义做全局唯一是最省事、最一致
   的做法。除非产品明确要"同一邮箱在不同租户下是两个独立身份"这种场景（类似
   一个人用同一邮箱加入多个 SaaS 组织但各自是独立账号），否则没必要引入更复杂
   的跨租户唯一性设计。
2. **自助注册租户重名处理** → 推荐**允许重名**。类比 Slack/Notion 的"创建工作
   空间"流程：同名工作空间在这类产品里是被允许的，靠内部 ID 区分，不靠名字唯一
   性防混淆。租户已经有 UUID 主键，业务上不依赖名字唯一；强制唯一只会增加"这个
   名字被占用了"的注册摩擦，没有实际防混淆价值（用户认自己的租户是靠登录后的
   归属关系，不是靠抢注一个全局唯一的名字）。
4. **`username` 列去留** → 推荐**保留作为显示名，与登录解耦**。登录统一走
   `email`（Supabase Auth 语义如此），但很多用户不希望界面上处处显示邮箱地址，
   `username` 作为纯显示用途保留有产品体验价值，废弃它只有损失、没有技术收益。
5. **触发器 vs 应用层写 `app_metadata.tenant_id`** → 推荐**应用层异步调用**（
   `signUp()` 成功后紧接着调一次 Admin API），而非数据库触发器直接发 HTTP 请求。
   理由：触发器方式需要 `pg_net`/HTTP 扩展从数据库直接出站调用外部 API，这在多数
   托管 Postgres（含 Supabase Cloud）里是较少见、需要额外网络出站白名单和权限配置
   的做法，运维风险和调试难度更高；应用层方式虽然存在"Auth 建号成功但 Admin API
   调用失败"的中间态，但这个中间态可以用一个简单的补偿检查兜底（登录时若
   `app_metadata.tenant_id` 为空就重试写入一次），整体更可控，也更符合本项目现有
   的六边形架构原则——数据库只负责数据完整性，编排逻辑放在应用层。
