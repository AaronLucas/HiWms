...
## 4.17 缺码补码闭环（Layer 4：MISSING_LABEL）

### 4.17.1 生成内部码
```http
POST /api/v1/device/missing-label/generate
Content-Type: application/json
...
```
底层封装 `fn_generate_internal_lpn`，生成格式为 `INT-{日期}-{随机串}` 的内部 LPN 码（`containers.lpn_source = 'SYSTEM_GENERATED'`），与供应商自带的 `EXTERNAL` 码区分。
...
### 4.17.2 确认贴码
```http
POST /api/v1/device/missing-label/confirm
Content-Type: application/json
...
```
底层封装 `fn_confirm_label_applied`，核对扫描码与生成码一致后，将暂存的库存正式挂载到该容器，并通过统一恢复入口 `fn_resolve_exception` 关闭异常。
...
## 4.18 无码/未识别货物闭环（Layer 4：UNIDENTIFIED_GOODS）

### 4.18.1 上报未识别货物
```http
POST /api/v1/device/unidentified/receive
Content-Type: application/json
...
```
底层封装 `fn_receive_unidentified_goods`，向 `inventory` 插入 `product_id=NULL` 的暂存行，登记 `UNIDENTIFIED_GOODS` 异常（默认严重度 `HIGH`，高于 `MISSING_LABEL` 的 `MEDIUM`，因为连基本身份都不确定，风险更高）。
...
### 4.18.2 识别未识别货物
```http
POST /api/v1/device/unidentified/identify
Content-Type: application/json
...
```
底层封装 `fn_identify_unidentified_goods`，回填 `product_id`，**同时触发合规触发器复查（UPDATE OF product_id 触发 `fn_trg_enforce_product_constraints`）**。若目标库位不满足该商品的冷链/危险品要求，识别动作会被直接拦下并报错，倒逼主管先把货移到合规库位，再回来确认身份；换成一个没有存储限制的普通商品，识别流程能正常走通并关闭异常。
...
{
  "success": false,
  "message": "识别失败：商品 SKU-000445 要求冷链存储（2-8°C），但目标库位 loc-uuid-Q01-01-01 非冷藏区，请先将货物移至合规库位后再确认身份"
}
```

---

## 4.19 上架（PUTAWAY，Layer 3 同步动作扩展）
```http
POST /api/v1/device/inbound/putaway
Content-Type: application/json
...
```
若上架暂存位与商品的合规属性冲突（如冷链商品被扫到常温暂存区），按合规执行说明处理：在线时硬阻断，离线回放时登记 `COLD_CHAIN_VIOLATION`/`HAZMAT_CONFLICT` 异常（见 3.9）。

---

## 4.20 盘点（COUNT，Layer 3 同步动作扩展）
```http
POST /api/v1/device/inventory/count/submit
Content-Type: application/json
...
## 4.21 打包/封箱（PACK，Layer 3 同步动作扩展）
```http
POST /api/v1/device/outbound/pack
Content-Type: application/json
...
```
打包完成时正确联动更新 `order_lines.status = 'PACKED'`，`packing_tasks.status = 'COMPLETED'`，同时写入 `packing_task_items` 明细（支持一箱一码/同码两种模式）。

---

## 4.22 缺码补码闭环（Layer 4：MISSING_LABEL）
...
## 4.23 无码/未识别货物闭环（Layer 4：UNIDENTIFIED_GOODS）
...
| 2.1.0 | 2026-07-16 | **Layer 3/4 扩展**：新增 PUTAWAY/COUNT/PACK 三个动作类型（Layer 3 同步动作扩展）、Layer 4 追踪策略/无码货物端点（`/missing-label/generate`、`/missing-label/confirm`、`/unidentified/receive`、`/unidentified/identify`），对应 Layer 3 同步动作扩展与 Layer 4 追踪策略/无码货物闭环 | DBA 团队 / 架构组 |
