...
- **3PL-Q019（OPEN/BLOCKING）**：owner 未知、有争议、标签损坏、盘盈的货由谁保管、
  ...
  **业务侧候选方案（2026-09-06 补充，用户提议 + 行业实践验证，非 DBA 侧已确认）**：
  用户提出可以靠差异化计费解决"哪些货能混放/哪些不能"：①能混放但没贴标的，收
  贴签费；②不贴标又需要专属存放的，按更低利用率收更高存储费。经查证（Extensiv/
  Racklify/PackageX/Amazon FBA 公开资料），该思路方向正确且有真实先例——Amazon
  FBA "Unplanned Services" 就是同一逻辑（约 $0.20-0.40/件代贴标费）。但**这套
  计费方案的前提是"货权已确权"，不能替代确权本身**：行业标准流程是先用
  ASN（预先到货清单）+条码/箱号扫描比对+异常拍照存证三重验证确认归属，验证
  通过后才谈"贴签费 vs 专属存放"这两种计费路径；验证失败（无 ASN 匹配、条码
  扫不出、无法拍照溯源）的才是真正的"孤儿库存"，应先进隔离区，走登记+通知
  发货方+催告期（建议参照合同约定 15-30 天）+超期按合同处置的流程，不能直接
  套用贴签费流程。也就是说：**"确权"和"计费"是两个先后步骤，不是同一个方案**。
  完整流程建议：
  ```
  到货 → ASN/条码/单据能确认归属？
    ├─ 能 → 归属已确权，按用户思路计费：
    │        无标签但能混放 → 收贴签费
    │        无标签且需专属存放 → 收更高存储费
    └─ 不能 → 先进隔离区（孤儿库存流程），不能直接贴签/混放
  ```
  标签损坏的常见诱因（Avery/PLCTX/WareIQ）：运输环节雨淋导致热敏纸油墨晕染、
  反复装卸摩擦致不干胶脱落；仓储环节阳光直射/粉尘、纸箱回收利用留下旧标签
