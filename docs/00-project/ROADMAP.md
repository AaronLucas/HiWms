...
| **CRITICAL** | 登录/注册身份模型分裂 + RLS 租户上下文从未真正注入查询连接（2026-07-20 排期任务 #5 执行过程中发现，详见 ADR-015） | `REPOSITORY_ROADMAP.md` §8「剩余缺口清单」#7；`docs/01-architecture/ADR/015-auth-identity-bridge.md`；`.readonly/unWMS_Auth_Identity_Bridge_V1.md` | 设计已完成（方案 A），待评审后实施，阻塞本文件「登录/鉴权」模块（阶段 2 §2.2）与排期任务 #5 |
