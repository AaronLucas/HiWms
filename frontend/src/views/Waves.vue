<template>
  <div class="waves-page">
    <div class="page-header">
      ...
      <el-button type="primary" @click="openGenerateDialog" :loading="submitLoading">
    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item label="波次编号">
          <el-input v-model="searchForm.waveNo" placeholder="波次编号" clearable @keyup.enter="handleSearch" />
        </el-form-item>
        ...
        <el-form-item label="仓库">
          <el-select v-model="searchForm.warehouseId" clearable style="width: 200px" placeholder="请选择仓库" filterable remote :remote-method="remoteWarehouseSearch" :loading="warehouseLoading">
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="tableData" row-key="id" border style="width: 100%" @row-dblclick="openDetailDialog">
        ...
        <el-table-column label="释放时间" width="160"><template #default="scope">{{ formatDate(scope.row.releasedAt) }}</template></el-table-column>
        <el-table-column label="完成时间" width="160"><template #default="scope">{{ formatDate(scope.row.completedAt) }}</template></el-table-column>
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="scope">
            <el-button size="small" type="primary" link @click="openDetailDialog(scope.row)">详情</el-button>
            <el-divider direction="vertical" />
            <el-button size="small" type="success" link @click="releaseWave(scope.row)" v-if="scope.row.status === 'draft'">释放</el-button>
            <el-divider direction="vertical" v-if="scope.row.status === 'draft'" />
            <el-button size="small" type="danger" link @click="cancelWave(scope.row)" v-if="scope.row.status === 'draft'">取消</el-button>
            <el-divider direction="vertical" />
    ...
    <el-dialog v-model="generateDialogVisible" :title="generateDialogTitle" width="600px" :before-close="handleGenerateDialogClose" destroy-on-close>
      <el-form :model="generateForm" :rules="generateRules" ref="generateFormRef" label-width="120px">
        <el-form-item label="仓库" prop="warehouseId">
          <el-select v-model="generateForm.warehouseId" placeholder="请选择仓库" filterable remote :remote-method="remoteWarehouseSearch" :loading="warehouseLoading" style="width: 100%">
            <el-option v-for="w in warehouseOptions" :key="w.id" :label="w.name" :value="w.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="最大订单数" prop="maxOrders"><el-input-number v-model="generateForm.maxOrders" :min="1" :max="100" :step="1" controls-position="right" placeholder="默认 50" /></el-form-item>
        <el-form-item label="最大数量" prop="maxQty"><el-input-number v-model="generateForm.maxQty" :min="1" :step="1" controls-position="right" placeholder="默认不限制" /></el-form-item>
        <el-form-item label="仅分配可用库存" prop="onlyAllocated"><el-switch v-model="generateForm.onlyAllocated" active-value="true" inactive-value="false" /></el-form-item>
      </el-form>
      <template #footer><div class="dialog-footer"><el-button @click="generateDialogVisible = false">取消</el-button><el-button type="primary" :loading="submitLoading" @click="handleGenerate">生成</el-button></div></template>
    </el-dialog>
      ...
      <el-card shadow="never" v-if="detailData">
        <div class="detail-section">
          ...
          <el-descriptions :column="4" border>
            ...
            <el-descriptions-item label="释放时间">{{ formatDate(detailData.releasedAt) }}</el-descriptions-item>
            <el-descriptions-item label="完成时间">{{ formatDate(detailData.completedAt) }}</el-descriptions-item>
          </el-descriptions>
        </div>
        <div class="detail-section">
          <div class="section-header">
            <h4>包含订单</h4>
            <el-button size="small" @click="openAddOrdersDialog(detailData)" v-if="detailData.status === 'draft'"><el-icon><Plus /></el-icon> 添加订单</el-button>
          </div>
          <el-table :data="detailData.orders" border style="width: 100%">
            ...
            <el-table-column prop="allocatedQty" label="已分配" width="100" align="right" />
            <el-table-column label="状态" width="120"><template #default="scope"><el-tag :type="orderStatusTypeMap[scope.row.status]" size="small" effect="dark">{{ orderStatusLabelMap[scope.row.status] }}</el-tag></template></el-table-column>
            <el-table-column label="操作" width="100" align="center" v-if="detailData.status === 'draft'">
              <template #default="scope">
                <el-button size="small" type="danger" link @click="removeOrderFromWave(detailData.id, scope.row.id)"><el-icon><Delete /></el-icon></el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-dialog v-model="addOrdersDialogVisible" :title="addOrdersDialogTitle" width="800px" destroy-on-close>
      <el-form :model="addOrdersForm" ref="addOrdersFormRef" label-width="120px">
        <el-form-item label="可选订单">
          <el-table :data="availableOrders" v-loading="addOrdersLoading" row-key="id" border style="width: 100%" @selection-change="addOrdersSelectionChange">
      ...
      <template #footer><div class="dialog-footer"><el-button @click="addOrdersDialogVisible = false">取消</el-button><el-button type="primary" :loading="addOrdersSubmitLoading" @click="handleAddOrders" :disabled="addOrdersSelectedIds.length === 0">添加选中订单</el-button></div></template>
    </el-dialog>
  </div>
</template>
...
const statusTypeMap: Record<string, string> = { draft: "info", released: "primary", completed: "success", cancelled: "danger" };
const statusLabelMap: Record<string, string> = { draft: "草稿", released: "已释放", completed: "已完成", cancelled: "已取消" };
const orderStatusTypeMap: Record<string, string> = { pending: "info", allocated: "primary", picking: "warning", packed: "success", shipped: "success", cancelled: "danger" };
const orderStatusLabelMap: Record<string, string> = { pending: "待处理", allocated: "已分配", picking: "拣货中", packed: "已打包", shipped: "已发货", cancelled: "已取消" };
...
async function fetchList() {
  ...
  } catch (error) { console.error("Failed to fetch waves:", error); tableData.value = []; pagination.total = 0; }
  finally { loading.value = false; }
}

function handleSearch() { pagination.page = 1; fetchList(); }
function handleReset() { searchForm.waveNo = ""; searchForm.status = ""; searchForm.warehouseId = ""; pagination.page = 1; fetchList(); }
...
function resetGenerateForm() { generateForm.warehouseId = ""; generateForm.strategy = "priority"; generateForm.maxOrders = 50; generateForm.maxQty = 0; generateForm.onlyAllocated = true; generateFormRef.value?.clearValidate?.(); }
function handleGenerateDialogClose(done: () => void) { resetGenerateForm(); done(); }
...
async function handleDelete(id: string) {
  try {
    await ElMessageBox.confirm("确定要删除该波次吗？", "确认删除", { confirmButtonText: "确定", cancelButtonText: "取消", type: "warning" });
...
async function cancelWave(row: any) {
  try {
    await ElMessageBox.confirm("确定要取消该波次吗？", "确认取消", { confirmButtonText: "确定", cancelButtonText: "取消", type: "warning" });
...
async function openDetailDialog(row: any) {
  ...
  } catch (error) { console.error("Failed to fetch wave detail:", error); }
}
...
async function openAddOrdersDialog(wave: any) {
  ...
  try {
    const response: any = await api.get(ENDPOINTS.ORDERS_LIST, { status: "pending,allocated", limit: 100 });
    availableOrders.value = (response.data as any[]) || [];
  } catch (error) { console.error("Failed to fetch available orders:", error); availableOrders.value = []; }
  finally { addOrdersLoading.value = false; }
}
...
async function handleAddOrders() {
  ...
  } catch (error) { console.error("Add orders error:", error); }
  finally { addOrdersSubmitLoading.value = false; }
}

async function removeOrderFromWave(waveId: string, orderId: string) {
  try {
    await ElMessageBox.confirm("确定要从波次中移除该订单吗？", "确认移除", { confirmButtonText: "确定", cancelButtonText: "取消", type: "warning" });
    await api.delete(ENDPOINTS.WAVES_REMOVE_ORDER(waveId, orderId));
    ElMessage.success("移除成功");
    await openDetailDialog(detailData.value);
  } catch (error) { if (error !== "cancel") console.error("Remove order error:", error); }
}
