<template>
  <div class="inventory-page">
    <div class="page-header">
      ...
      <el-button type="primary" @click="openAdjustDialog" :loading="submitLoading">
        <el-icon><Edit /></el-icon>
        库存调整
      </el-button>
      <el-button type="success" @click="openTransferDialog" :loading="submitLoading">
        <el-icon><Transfer /></el-icon>
        移库
      </el-button>
      <el-button type="info" @click="openReserveDialog" :loading="submitLoading">
    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item label="商品">
          <el-input v-model="searchForm.productSku" placeholder="SKU/名称" clearable @keyup.enter="handleSearch" />
        </el-form-item>
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="tableData" row-key="id" border style="width: 100%">
        ...
        <el-table-column prop="quantity" label="数量" width="100" align="right">
          <template #default="scope"><span :class="scope.row.quantity <= 0 ? 'text-danger' : ''">{{ scope.row.quantity }}</span></template>
        </el-table-column>
        ...
        <el-table-column label="效期" width="140">
          <template #default="scope"><span v-if="scope.row.expiryDate">{{ formatDate(scope.row.expiryDate) }}</span><span v-else class="text-muted">无效期</span></template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="scope">
            <el-button size="small" type="primary" link @click="openAdjustDialog(scope.row)">调整</el-button>
            <el-divider direction="vertical" />
            <el-button size="small" type="info" link @click="openTransferDialog(scope.row)">移库</el-button>
            <el-divider direction="vertical" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" :before-close="handleDialogClose" destroy-on-close>
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        <el-form-item label="商品" prop="productId"><el-select v-model="form.productId" placeholder="请选择商品" filterable remote :remote-method="remoteProductSearch" :loading="productLoading" style="width: 100%"><el-option v-for="p in productOptions" :key="p.id" :label="p.sku + ' - ' + p.name" :value="p.id" /></el-select></el-form-item>
        <el-form-item label="库位" prop="locationId"><el-select v-model="form.locationId" placeholder="请选择库位" filterable remote :remote-method="remoteLocationSearch" :loading="locationLoading" style="width: 100%"><el-option v-for="l in locationOptions" :key="l.id" :label="l.code" :value="l.id" /></el-select></el-form-item>
        <el-form-item label="批次号" prop="batchNo"><el-input v-model="form.batchNo" placeholder="可选" maxlength="50" clearable /></el-form-item>
        <el-form-item label="序列号" prop="serialNo"><el-input v-model="form.serialNo" placeholder="可选（唯一追踪商品必填）" maxlength="100" clearable /></el-form-item>
        <el-form-item label="数量" prop="quantity"><el-input-number v-model="form.quantity" :min="1" :step="1" controls-position="right" placeholder="请输入数量" /></el-form-item>
        <el-form-item label="效期" prop="expiryDate"><el-date-picker v-model="form.expiryDate" type="date" placeholder="可选" value-format="YYYY-MM-DD" clearable /></el-form-item>
        <el-form-item label="操作类型" v-if="dialogType === 'adjust'"><el-radio-group v-model="form.adjustType"><el-radio label="increase">增加</el-radio><el-radio label="decrease">减少</el-radio><el-radio label="set">设定</el-radio></el-radio-group></el-form-item>
        <el-form-item label="目标库位" prop="toLocationId" v-if="dialogType === 'transfer'"><el-select v-model="form.toLocationId" placeholder="请选择目标库位" filterable remote :remote-method="remoteLocationSearch" :loading="toLocationLoading" style="width: 100%"><el-option v-for="l in locationOptions" :key="l.id" :label="l.code" :value="l.id" /></el-select></el-form-item>
        <el-form-item label="备注" prop="reason"><el-input v-model="form.reason" type="textarea" :rows="2" placeholder="请输入原因" maxlength="200" /></el-form-item>
      </el-form>
    ...
    <el-dialog v-model="reserveDialogVisible" :title="reserveDialogTitle" width="500px" destroy-on-close>
      <el-form :model="reserveForm" :rules="reserveRules" ref="reserveFormRef" label-width="100px">
        <el-form-item label="商品" prop="productId"><el-select v-model="reserveForm.productId" placeholder="请选择商品" filterable remote :remote-method="remoteProductSearch" :loading="productLoading" style="width: 100%"><el-option v-for="p in productOptions" :key="p.id" :label="p.sku + ' - ' + p.name" :value="p.id" /></el-select></el-form-item>
        <el-form-item label="库位" prop="locationId"><el-select v-model="reserveForm.locationId" placeholder="请选择库位" filterable remote :remote-method="remoteLocationSearch" :loading="locationLoading" style="width: 100%"><el-option v-for="l in locationOptions" :key="l.id" :label="l.code" :value="l.id" /></el-select></el-form-item>
        <el-form-item label="数量" prop="quantity"><el-input-number v-model="reserveForm.quantity" :min="1" :step="1" controls-position="right" /></el-form-item>
        <el-form-item label="关联单据" prop="orderId"><el-input v-model="reserveForm.orderId" placeholder="订单/工单 ID（可选）" maxlength="50" clearable /></el-form-item>
        <el-form-item label="过期时间" prop="expiresAt"><el-date-picker v-model="reserveForm.expiresAt" type="datetime" placeholder="可选" value-format="YYYY-MM-DD HH:mm:ss" clearable /></el-form-item>
      </el-form>
      <template #footer><el-button @click="reserveDialogVisible = false">取消</el-button><el-button type="primary" :loading="reserveSubmitLoading" @click="handleReserveSubmit">{{ reserveDialogType === 'reserve' ? '预留' : '锁定' }}</el-button></template>
    </el-dialog>
    ...
    <el-dialog v-model="historyDialogVisible" title="库存变动历史" width="900px" destroy-on-close>
      <el-table :data="historyData" border style="width: 100%" row-key="id">
        ...
        <el-table-column prop="changeQty" label="变动量" width="100" align="right"><template #default="scope"><span :class="scope.row.changeQty > 0 ? 'text-success' : 'text-danger'">{{ scope.row.changeQty > 0 ? '+' : '' }}{{ scope.row.changeQty }}</span></template></el-table-column>
        <el-table-column prop="changeType" label="变动类型" width="140"><template #default="scope"><el-tag size="small" effect="plain">{{ scope.row.changeType }}</el-tag></template></el-table-column>
        <el-table-column prop="reason" label="原因" min-width="200" show-overflow-tooltip />
        <el-table-column prop="referenceId" label="关联单据" width="160" show-overflow-tooltip />
        <el-table-column prop="createdAt" label="时间" width="180"><template #default="scope">{{ formatDate(scope.row.createdAt) }}</template></el-table-column>
      </el-table>
    </el-dialog>
  </div>
</template>
...
import { Plus, Search, Refresh, Edit, Transfer, Lock, Delete, History, Box, ArrowRight, ArrowLeft, Time } from "@element-plus/icons-vue";
...
const formRef = ref();
const form = reactive({ id: "", productId: "", locationId: "", toLocationId: "", batchNo: "", serialNo: "", quantity: 1, expiryDate: "", adjustType: "increase", reason: "" });

const rules = { productId: [{ required: true, message: "请选择商品", trigger: "change" }], locationId: [{ required: true, message: "请选择库位", trigger: "change" }], quantity: [{ required: true, message: "请输入数量", trigger: "blur" }, { min: 1, message: "数量必须大于 0", trigger: "blur" }], reason: [{ required: true, message: "请输入原因", trigger: "blur" }], toLocationId: [{ required: true, message: "请选择目标库位", trigger: "change" }] };
...
async function fetchList() {
  ...
  } catch (error) { console.error("Failed to fetch inventory:", error); tableData.value = []; pagination.total = 0; }
...
async function handleSubmit() {
  formRef.value?.validate?.(async (valid: boolean) => {
    ...
    try {
      if (dialogType.value === "adjust") { const payload = { productId: form.productId, locationId: form.locationId, batchNo: form.batchNo || undefined, serialNo: form.serialNo || undefined, quantity: form.quantity, expiryDate: form.expiryDate || undefined, adjustType: form.adjustType, reason: form.reason }; await api.post(ENDPOINTS.INVENTORY_ADJUST, payload); ElMessage.success("库存调整成功"); }
      else if (dialogType.value === "transfer") { const payload = { productId: form.productId, fromLocationId: form.locationId, toLocationId: form.toLocationId, batchNo: form.batchNo || undefined, serialNo: form.serialNo || undefined, quantity: form.quantity, reason: form.reason }; await api.post(ENDPOINTS.INVENTORY_TRANSFER, payload); ElMessage.success("移库成功"); }
...
async function handleDelete(id: string) {
  try { await ElMessageBox.confirm("确定要删除该库存记录吗？", "确认删除", { confirmButtonText: "确定", cancelButtonText: "取消", type: "warning" }); await api.delete(ENDPOINTS.INVENTORY_GET(id)); ElMessage.success("删除成功"); fetchList(); } catch (error) { if (error !== "cancel") console.error("Delete error:", error); }
}
