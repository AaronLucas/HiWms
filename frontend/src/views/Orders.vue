  ...
  <div class="orders-page">
    ...
    <el-card shadow="never">
      ...
      >
        ...
        >
        <el-table-column label="要求发货时间" width="160"
          ><template #default="scope">{{
            formatDate(scope.row.requiredShipDate)
          }}</template></el-table-column
        >
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="scope">
            ...
            >
            <el-divider
              v-if="scope.row.status === 'pending' || scope.row.status === 'allocated'"
              direction="vertical"
            />
            <el-button
              @click="allocateOrder(scope.row)"
              v-if="scope.row.status === 'pending' || scope.row.status === 'allocated'"
              size="small"
              type="warning"
              link
            ...
            >
            <el-divider
              v-if="scope.row.status !== 'cancelled' && scope.row.status !== 'shipped'"
              direction="vertical"
            />
            <el-button
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item v-if="form.id" label="订单明细">
          <div class="order-lines">
            <div class="lines-header">
              <span>商品明细</span>
              <el-button @click="addOrderLine" size="small"
              ...
              <el-table-column label="数量" width="100" align="right">
                <template #default="scope">
                  <el-input-number
                    ...
                    :min="1"
                    :step="1"
                    @change="updateOrderLine(scope.$index, scope.row)"
        ...
        <div class="detail-section">
          ...
          <el-descriptions :column="4" border>
            ...
            <el-descriptions-item label="优先级"
              ><el-tag :type="priorityTypeMap[detailData.priority]" effect="dark">{{
                priorityLabelMap[detailData.priority]
              }}</el-tag></el-descriptions-item
            >
            ...
            <el-descriptions-item label="要求发货时间">{{
              formatDate(detailData.requiredShipDate)
            }}</el-descriptions-item>
...
function openDialog(row?: any) {
  ...
  if (row) {
    ...
    orderLines.value = [
      { productId: '', productSku: '', productName: '', qty: 1, allocatedQty: 0, shippedQty: 0 },
    ];
  }
...
function updateOrderLine(index: number, line: any) {
  /* 可在此添加校验逻辑 */
}
