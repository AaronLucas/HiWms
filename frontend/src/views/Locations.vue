<template>
  <div class="locations-page">
      ...
      >
        ...
        <el-table-column label="类型" width="120">
          <template #default="scope"
            ><el-tag :type="typeTypeMap[scope.row.type]" size="small" effect="dark">{{
            ...
            <el-progress
              :percentage="
                scope.row.maxCapacity
                  ? Math.round(((scope.row.currentUsage || 0) / scope.row.maxCapacity) * 100)
                  : 0
              "
              :stroke-width="16"
            />
        ...
        <el-table-column label="允许混储" width="100" align="center">
          <template #default="scope"
            ><el-tag v-if="scope.row.allowMixed" type="success" size="small">是</el-tag
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="scope">
            ...
            >
            <el-divider direction="vertical" />
            <el-button @click="openCapacityDialog(scope.row)" size="small" type="info" link
              >容量</el-button
            >
            <el-divider direction="vertical" />
            <el-popconfirm
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="库位编码" prop="code"
          ><el-input v-model="form.code" placeholder="请输入库位编码" maxlength="50" clearable
        /></el-form-item>
        <el-form-item label="库位名称" prop="name"
          ><el-input v-model="form.name" placeholder="请输入库位名称" maxlength="100" clearable
        /></el-form-item>
        ...
        /></el-form-item>
        <el-form-item label="允许混储" prop="allowMixed"
          ><el-switch v-model="form.allowMixed" active-value="true" inactive-value="false"
        /></el-form-item>
        <el-form-item label="允许拆分" prop="allowSplit"
          ><el-switch v-model="form.allowSplit" active-value="true" inactive-value="false"
        /></el-form-item>
    ...
    <el-dialog
      ...
      destroy-on-close
    >
      <el-card v-if="capacityData" shadow="never">
        <div class="capacity-info">
          ...
          <div class="capacity-row">
            ...
            <span class="value">
              <el-progress
                :percentage="
                  capacityData.maxCapacity
                    ? Math.round((capacityData.currentUsage / capacityData.maxCapacity) * 100)
                    : 0
                "
                :stroke-width="16"
              />
      ...
      <template #footer
        ><el-button @click="capacityDialogVisible = false">关闭</el-button></template
      >
    </el-dialog>
  </div>
</template>
...
const rules = {
  ...
  maxCapacity: [
    ...
    { min: 0, message: '容量不能为负数', trigger: 'blur' },
  ],
...
function resetForm() {
  ...
  form.maxCapacity = 0;
  form.priority = 0;
  form.allowMixed = false;
  form.allowSplit = false;
  form.description = '';
...
async function handleDelete(id: string) {
  try {
    await ElMessageBox.confirm('确定要删除该库位吗？', '确认删除', {
...
.capacity-row .value .el-progress {
  width: 150px;
}
</style>
